...
class P_FastForm : public QObject
{
	...
	protected:
		...
		virtual void _addSingle(C_FastClassParam c_fp, QGridLayout* layout, int curRow, int curCol);
										//布局 - 并列控件
		virtual void _addOneLine(C_FastClass c_fc, QGridLayout* layout, int curRow, int curCol);
										//布局 - 群组控件
		virtual void _addQGroupBox(C_FastClass c_fc, QGridLayout* layout, int curRow, int curCol);
										//布局 - 块组控件
		virtual void _addQWidgetGroup(C_FastClass c_fc, QGridLayout* layout, int curRow, int curCol);
	protected:
										//构建 - 表单组（没有则返回nullptr）
